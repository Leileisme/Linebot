...
export const update = async () => {
  ...
  try {
      ...
      $('#vege_chart').each(function (index) {
        ...
        for (let j = 1; j <= trFruit; j++) {
          ...
          if (title.includes($F('tbody').find('tr').eq(j).find('td').eq(2).text())) {
            ...
            // console.log(j, 'if裡面的成功J')
            // console.log(title, '抬頭')
            // console.log($F('tbody').find('tr').eq(j).find('td').eq(2).text(), '預警品種')
            // console.log($F('tbody').find('tr').eq(j).find('td').eq(2).text(), priceF, j)
            break
          } else {
    ...
    // 找出預警蔬菜
    // const { data: dataV } = await axios.post('https://www.tapmc.com.tw/Pages/Trans/Signal', {
    //   ctl00$ContentPlaceHolder1$txtDate: '112/12/01',
    ...
    // console.log('Vegetable的tr長度' + trVegetable)
    // console.log($V('tbody').find('tr').eq(1).find('td').eq(2).text(), '$V蔬菜的title')
    ...
    for (let i = 1; i <= 57; i++) {
      ...
      $('#vege_chart').each(function (index) {
        ...
        //   if (title.includes($V('tbody').find('tr').eq(j).find('td').eq(2).text())) {
        //     const priceV = $V('tbody').find('tr').eq(j).find('td').eq(5).text()
        //     console.log(title)
        //     console.log($V('tbody').find('tr').eq(j).find('td').eq(2).text())

        //     template.body.contents[1].contents[2].contents[1].text = '警戒'
        //     template.body.contents[1].contents[2].contents[1].color = '#ff0000'
        //     template.body.contents[1].contents[2].contents[1].size = 'lg'
        //     template.body.contents[1].contents[3].contents[1].text = priceV * 0.6
        //   } else {
        //     template.body.contents[1].contents[2].contents[1].text = '無警戒'
        //     template.body.contents[1].contents[2].contents[1].color = '#cccccc'
        //     template.body.contents[1].contents[2].contents[1].size = 'md'
