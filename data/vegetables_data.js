    ...
    for (let i = 1; i <= 31; i++) {
      ...
      $('#vege_chart').each(function (index) {
        ...
        for (let j = 1; j <= trFruit; j++) {
          if (
            title.includes($F('tbody').find('tr').eq(j).find('td').eq(1).text()) &&
            title.includes($F('tbody').find('tr').eq(j).find('td').eq(2).text())
          ) {
            const priceF = (Math.round($F('tbody').find('tr').eq(j).find('td').eq(5).text() * 0.6 * 10) / 10).toString()
    ...
    for (let i = 1; i <= 57; i++) {
      ...
      $('#vege_chart').each(function (index) {
        ...
        for (let j = 1; j <= trVegetable; j++) {
          if (
            title.includes($V('tbody').find('tr').eq(j).find('td').eq(1).text()) &&
            title.includes($V('tbody').find('tr').eq(j).find('td').eq(2).text())
          ) {
            const priceV = (Math.round($V('tbody').find('tr').eq(j).find('td').eq(5).text() * 0.6 * 10) / 10).toString()
            template.body.contents[1].contents[2].contents[1].text = '警戒'
            template.body.contents[1].contents[2].contents[1].color = '#ff0000'
            template.body.contents[1].contents[2].contents[1].size = 'lg'
            template.body.contents[1].contents[3].contents[1].text = priceV
            break
          } else {
            template.body.contents[1].contents[2].contents[1].text = '無警戒'
            template.body.contents[1].contents[2].contents[1].color = '#cccccc'
